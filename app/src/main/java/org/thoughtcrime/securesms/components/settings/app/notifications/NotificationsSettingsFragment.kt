...
class NotificationsSettingsFragment : DSLSettingsFragment(R.string.preferences__notifications) {
  ...
  private fun getConfiguration(state: NotificationsSettingsState): DSLConfiguration {
    return configure {
      ...
      val notificationMethodValues = NotificationDeliveryMethod.entries.filterNot { method ->
        method == NotificationDeliveryMethod.FCM && !BuildConfig.USE_PLAY_SERVICES
        method == NotificationDeliveryMethod.UNIFIEDPUSH && state.isLinkedDevice
      }
      val notificationMethodLabels by lazy { notificationMethodValues.map {
        resources.getString(it.stringId) }.toTypedArray()
      }
