...
public final class MessageFetchJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    ...
    if (Build.VERSION.SDK_INT < 31) {
      try (DelayedNotificationController unused = GenericForegroundService.startForegroundTaskDelayed(context, context.getString(R.string.BackgroundMessageRetriever_checking_for_messages), 300, R.drawable.ic_notification)) {
        success = WebSocketDrainer.blockUntilDrainedAndProcessed();
      }
    } else {
      try {
        try (NotificationController unused = GenericForegroundService.startForegroundTask(context, context.getString(R.string.BackgroundMessageRetriever_checking_for_messages), NotificationChannels.getInstance().OTHER, R.drawable.ic_notification)) {
          success = WebSocketDrainer.blockUntilDrainedAndProcessed();
        }
