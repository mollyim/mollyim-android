  ...
  private fun updateReleaseNotes(manifestMd5: ByteArray) {
    ...
    if (allReleaseNotes != null) {
      val resolvedNotes: List<FullReleaseNote?> = allReleaseNotes.announcements
        .filter { it.androidMinVersion.toIntOrNull()?.let { minVersion: Int -> minVersion > values.highestVersionNoteReceived && minVersion <= BuildConfig.SIGNAL_CANONICAL_VERSION_CODE } ?: false }
